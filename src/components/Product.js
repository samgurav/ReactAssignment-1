...
export class Product extends Component {
    ...
    componentDidMount(){
      this.setState({proData:products})
    ...
    addCart=(id,quantity)=>{
      ...
      if(localStorage.getItem('mycart') !== null){
        ...
        if(arr.some(e=>e.id === id)){
          ...
          alert('Product Already Added Only quantity is increasing!!')
        ...
        localStorage.setItem('mycart',JSON.stringify(arr))
        alert('Product Added to Cart!')
        // this.setState({arrlen:arr.length})
      }
    ...
    render() {
        return (
          ...
          <nav className="nav bg-dark" key={Math.random()}>
          <a key={Math.random()} className="nav-link " href=" ">Home</a>
          <a key={Math.random()} className="nav-link" href=" ">About</a>
          <a key={Math.random()} className="nav-link" href=" " onClick={()=>this.displayCart() } style={{background:'white', fontSize:'18px',color:'black'}}>Cart <span ><b >{this.state.arrlen}</b></span></a>
          <a key={Math.random()} className="nav-link " href=" ">Disabled</a>
          </nav>
            <h1 key={Math.random()}>Products</h1>
            <div className="row container-fluid m-auto " key={Math.random()}>
                {products.map(pro=>
                  <div className="card-body block col-sm-4  bg-light" key={Math.random()}>
                    ...
                    <div className="text-center">
                                   <button className="btn btn-success mr-100 " key={Math.random()} onClick={()=>this.addCart(pro.id,pro.quantity)} style={{marging:'100px'}} >ADD TO CART</button> </div>
                                              </div>)}
                   
            </div>
            <div key={Math.random()} className="table-responsivem container" id="carttable" style={{visibility:"visible"}}>
                  <table key={Math.random()} className="table table-hover table-bordered table-dark" border={3} style={{marginTop:'20px'}}>
                  <thead key="thead" >
                      <tr className="bg-info" key={Math.random()}>
                ...
                <tbody key="tbody" style={{background:'#E5E4E2' ,color:'black'}}>
